...
(() => {
  ...
  // Per-slot image bytes live in their own file so a big drop (an animated
  // GIF) is one small isolated write, not a rewrite of every slot's bytes.
  // STATE_FILE is now a small INDEX: framing metadata (s,x,y) + an `f` flag
  // meaning "image bytes are in the per-slot file". Legacy inline-`u` entries
  // (older sidecars) are still read and migrated to per-slot files on load.
  const IMG_DIR = '.image-slots/';
  // Flat root filenames — the host's writeFile does not create subdirectories,
  // so a nested path fails silently. One file per slot keeps each write small.
  const slotFile = (id) => '.imgslot-' + String(id).replace(/[^a-zA-Z0-9._-]/g, '_') + '.imgdata';
  // ids whose bytes are confirmed written to their per-slot file. Until an id
  // is here, the index keeps its bytes INLINE so a failed file write can never
  // lose the image.
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then(async (j) => {
        if (j && typeof j === 'object') {
          // Resolve each entry's image bytes: inline `u` (legacy) is used
          // as-is and flagged for migration; an `f`-flagged entry fetches
          // its per-slot file.
          await Promise.all(Object.keys(j).map(async (id) => {
            const e = j[id];
            const entry = typeof e === 'string'
              ? { u: e, s: 1, x: 0, y: 0 }
              : Object.assign({}, e);
            if (entry.u != null && entry.u !== '') {
              legacyIds.push(id); // inline bytes → migrate to per-slot file
            } else if (entry.f) {
              try {
                const rr = await fetch(slotFile(id));
                if (rr.ok) { entry.u = (await rr.text()).trim(); persisted.add(id); }
              } catch (_) {}
            }
            delete entry.f;
            j[id] = entry;
      ...
      .then(() => {
        loaded = true;
        subs.forEach((fn) => fn());
        // Migrate legacy inline-bytes sidecars to the per-slot layout: one
        // save writes each image to its own file and shrinks the index.
        if (legacyIds.length) {
          for (const id of legacyIds) if (slots[id]) dirtyImg.add(id);
          save();
        }
      });
    return loadP;
  ...
  let saving = false;
  let saveDirty = false;
  // ids whose image bytes changed and need their per-slot file rewritten.
  const dirtyImg = new Set();
  function indexPayload() {
    const out = {};
    for (const k in slots) {
      const v = slots[k];
      if (!v) continue;
      const o = typeof v === 'string' ? { u: v, s: 1, x: 0, y: 0 } : v;
      if (persisted.has(k) && o.u) out[k] = { s: o.s, x: o.x, y: o.y, f: 1 };
      else out[k] = { u: o.u, s: o.s, x: o.x, y: o.y }; // inline until file confirmed
    }
    return JSON.stringify(out);
  }
  function writeDirtyImages(w) {
    let p = Promise.resolve();
    const ids = Array.from(dirtyImg);
    dirtyImg.clear();
    for (const id of ids) {
      const v = slots[id];
      const u = v && (typeof v === 'string' ? v : v.u);
      if (!u) { persisted.delete(id); continue; }
      p = p.then(() => Promise.resolve(w(slotFile(id), u))
        .then(() => { persisted.add(id); })
        .catch(() => { dirtyImg.add(id); }));
    }
    return p;
  }
  function flushNow() {
    ...
    if (!w) return;
    try {
      for (const id of Array.from(dirtyImg)) {
        const v = slots[id];
        const u = v && (typeof v === 'string' ? v : v.u);
        if (u) w(slotFile(id), u);
      }
      w(STATE_FILE, indexPayload());
    } catch (e) {}
  }
  function save() {
    ...
    if (!w) return;
    saving = true;
    // Image files first (the bytes must exist before the index commits to
    // referencing them), then the tiny index as the commit. Until the index
    // is rewritten the previous one still resolves every slot, so a reload
    // mid-write never loses an image.
    writeDirtyImages(w)
      .then(() => Promise.resolve(w(STATE_FILE, indexPayload())).catch(() => {}))
      .then(() => { saving = false; if (saveDirty || dirtyImg.size) { saveDirty = false; save(); } });
  }
  ...
  function setSlot(id, val) {
    ...
    if (val) {
      const prev = slots[id];
      const prevU = prev && (typeof prev === 'string' ? prev : prev.u);
      const newU = typeof val === 'string' ? val : val.u;
      if (newU !== prevU) dirtyImg.add(id); // image bytes changed → rewrite its file
      slots[id] = val; tombstones.delete(id);
    }
    else { delete slots[id]; dirtyImg.delete(id); if (!loaded) tombstones.add(id); }
    subs.forEach((fn) => fn());
  ...
  async function toDataUrl(file, targetW) {
    // GIFs are passed through untouched: a canvas re-encode keeps only the
    // first frame, so an animated GIF would go still. Read raw bytes as a
    // data URL instead so the animation survives.
